...
def main():
    ...
    #FIXME
    copy_5_cur_state = retain_node(graphs)
    copy_5_cur_state_2 = retain_node(graphs)
    ...
    cur_state = retain_node(graphs)
    #base = new_node(graphs, construct(graphs, '1', B0, B1), '')
    graphs[cur_state][0] = new_node(graphs, B0, '')
    #cur_state = new_state_set(graphs, '1', '1'*15, B0, B1)
    # 29 さえあれば 1 をたどれば 31 を表せる! 1 でも0 でも関係ない ! cnt_graph 不要

    copy_5_cur_state = use_node(graphs, copy_5_cur_state, graphs[cur_state][0], copy5_state)
    copy_5_cur_state_2 = use_node(graphs, copy_5_cur_state_2, graphs[cur_state][0], copy5_state)
...
# cur_state と cur_state3 の内容は同じ -> なので引数部分のノード共通化できる
    ...
    base = get(graphs, genif_state, '0110110110')
    graphs[base][0] = construct(graphs, '1',0,0)
    graphs[graphs[cur_state][0]][1] = base
